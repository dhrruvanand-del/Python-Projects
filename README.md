# Academic Python Collection (Class 11 PCM)
![Your GitHub stats](https://github-readme-stats.vercel.app/api?username=dhrruvanand-del&show_icons=true&theme=radical)
This repository serves as a centralized workspace for the computational tools I develop alongside my Class 8,9,10 and 11 studies.

## 🚀 Purpose
The goal is to bridge the gap between theoretical Physics/Mathematics and practical programming logic. These scripts are used to verify complex calculations and simulate physical phenomena.

## 📂 Project Modules
* *Physics Simulations:* Projectile motion, Vector analysis, and Gravitational calculations.
* *Mathematical Utilities:* Trigonometric ratios and Quadratic solvers.
* *API Integration:* Real-time data fetching (Weather/ISS Tracking) using the requests library.
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=dhrruvanand-del&layout=compact&theme=radical)
## 🛠️ Environment
* *Language:* Python 3.12
* *IDE:* Visual Studio Code
* *Libraries:* math, random, requests
## 🔭 Current Focus
- 📚 **Academic:** Preparing for JEE/IAT with a focus on Theoretical Physics & Calculus.
- 💻 **Coding:** Developing simulations for classical mechanics and learning LaTeX for documentation.
- 🧪 **Research Interests:** String Theory and Computational Physics.
